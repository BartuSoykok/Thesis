...
class IROF_Blur(EvaluationMethod):
    ...
    def get_evaluation(self, X: np.ndarray) -> Tuple[float, List[Tuple[int, float]]]:
        ...
        best_pred = best_pred + (np.where(best_pred >= 0, 1., -1.) * 1e-30)
        ...
        segments = slic(X, n_segments=self.sen_N)
        ...
        for i in sorted_seg_ids:
            ...
            mask = segments == i
            blur_val = np.sum(X * mask.astype(int)) / (np.sum(mask.astype(int)) * 3)
            temp_X = np.where(mask, blur_val, temp_X)
            ...
            new_diff = best_pred_eps / best_pred
        ...
        display_results = [
            np.mean( np.trapz(1 - results[:, :i]) )
            for i in range(1, results.shape[1]+1)
        ]
        
        return display_results[-1], display_results
