...
# TODO Experiment with kernel
# TODO Autoencoders

class RemoveBestPixel(EvaluationMethod):
    ...
    def get_evaluation(self, X: np.ndarray) -> Tuple[float, List[Tuple[int, float]]]:
        ...
        kernel = np.zeros((11, 11, 1), dtype=np.double)
        ...
        for n in range(self.sen_N):
            ...
            temp_expl = np.where(temp_X_dash, temp_expl.min(), temp_expl)
            temp_X = np.where(temp_X_dash, temp_X.min(), temp_X)
